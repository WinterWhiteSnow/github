        ...
        if choice2 >= len(countries) or choice2 <0:
            ...
            try:
                ...
                if amount == str:
                    ...
                    print(f"{country1['code']} {amount} is {exchange_money}")
                    URL = f"https://wise.com/gb/currency-converter/{country1['code']}-to-{country2['code']}-rate?amount={money}"
                    request = requests.get(URL)
                    soup = BeautifulSoup(request.text, "html.parser")
                    rate = soup.find("h3", {"class":"cc__source-to-target"})
                    rate1 = rate.find_all("span")
                    result_rate = rate1[2].text
                    print(result_rate)
